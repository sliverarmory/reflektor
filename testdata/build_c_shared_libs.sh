...
for fixture in "basic:${SOURCE_FILE}" "args:${ARGS_SOURCE_FILE}"; do
  ...
  build_one "linux"   "arm"   "arm-linux-gnueabihf" "so"    "${base_name}" "${source_file}"
  build_one "linux"   "arm64" "aarch64-linux-gnu"   "so"    "${base_name}" "${source_file}"
  build_one "linux"   "ppc64le" "powerpc64le-linux-gnu" "so" "${base_name}" "${source_file}"
  build_one "linux"   "riscv64" "riscv64-linux-gnu" "so"    "${base_name}" "${source_file}"
...
build_recursive_one "linux"   "arm"   "arm-linux-gnueabihf" "so"
build_recursive_one "linux"   "arm64" "aarch64-linux-gnu"   "so"
build_recursive_one "linux"   "ppc64le" "powerpc64le-linux-gnu" "so"
build_recursive_one "linux"   "riscv64" "riscv64-linux-gnu" "so"
