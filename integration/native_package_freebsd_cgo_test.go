...
func TestNativePackageCSharedConsumerFreeBSD(t *testing.T) {
	consumerPath := buildNativeConsumerFreeBSDSharedLibrary(t)
...
func buildNativeConsumerFreeBSDSharedLibrary(t *testing.T) string {
	t.Helper()
	outputPath := filepath.Join(t.TempDir(), "native_consumer_freebsd-amd64.so")
	cmd := exec.Command("go",
		"build",
		"-buildmode=c-shared",
		"-trimpath",
