...
func TestNativePackageFreeBSDDependencyGraphIsIsolated(t *testing.T) {
	...
	for _, cgoEnabled := range []string{"0", "1"} {
		...
		t.Run("cgo-"+cgoEnabled, func(t *testing.T) {
			...
			assertNativeFreeBSDBridgeFiles(t, cgoEnabled, modulePath)
		})
	}
}

func assertNativeFreeBSDBridgeFiles(t *testing.T, cgoEnabled string, modulePath string) {
	t.Helper()
	...
	backend := packages[modulePath+"/native"]
	if !slices.Contains(backend.GoFiles, "backend_elf.go") {
		t.Fatalf("native GoFiles = %v, want backend_elf.go", backend.GoFiles)
	}
	elfLoader := packages[modulePath+"/native/internal/linuxmem"]
	if !slices.Contains(elfLoader.GoFiles, "loader_elf.go") {
		t.Fatalf("linuxmem GoFiles = %v, want loader_elf.go", elfLoader.GoFiles)
	}
	if !slices.Contains(elfLoader.GoFiles, "call_64.go") {
		t.Fatalf("freebsd/%s GoFiles = %v, want call_64.go", runtime.GOARCH, elfLoader.GoFiles)
	}
	if len(elfLoader.SFiles) != 0 {
		t.Fatalf("freebsd/%s unexpectedly selected assembly bridge files: %v", runtime.GOARCH, elfLoader.SFiles)
	}
}

func TestNativePackageCallExportWithArgsFreeBSD(t *testing.T) {
	requireCommand(t, "zig")

	fixturePath := buildArgumentSharedLib(t, t.TempDir(), "freebsd", runtime.GOARCH)
	...
	callback := purego.NewCallback(func(data uintptr, size int32) int32 {
		callbackCalls++
		var valid bool
		callbackOutput, valid = copyNativeCallbackPayloadFreeBSD(data, size)
		if !valid {
...
func TestNativePackageRustCallExportWithArgsFreeBSD(t *testing.T) {
	fixturePath := buildNativeRustArgumentFixtureFreeBSD(t)
	payload, err := os.ReadFile(fixturePath)
	if err != nil {
...
func TestNativePackageELFLifecycleFreeBSD(t *testing.T) {
	fixturePath := buildNativeLifecycleFixtureFreeBSD(t)
	payload, err := os.ReadFile(fixturePath)
	if err != nil {
	...
	if err != nil || state != constructorState {
		t.Fatalf("constructor state = %#x, err=%v, want %#x", state, err, constructorState)
	}
	boundState, err := library.CallExportWithArgs("ReflektorLifecycleBind", uintptr(unsafe.Pointer(unsafe.SliceData(record))))
	runtime.KeepAlive(record)
	if err != nil || boundState != constructorState {
		t.Fatalf("bind result = %#x, err=%v, want %#x", boundState, err, constructorState)
	}
	if got := nativeLifecycleRecordWordFreeBSD(record, 0); got != constructorState {
		t.Fatalf("bound lifecycle record = %#x, want %#x", got, constructorState)
	}
	if got := nativeLifecycleRecordWordFreeBSD(record, 1); got != 0 {
		t.Fatalf("pre-Close destructor count = %d, want 0", got)
	}
	if err := library.Close(); err != nil {
		t.Fatalf("Close lifecycle library: %v", err)
	}
	if got := nativeLifecycleRecordWordFreeBSD(record, 0); got != destructorSentinel {
		t.Fatalf("destructor sentinel = %#x, want %#x", got, destructorSentinel)
	}
	if got := nativeLifecycleRecordWordFreeBSD(record, 1); got != 1 {
...
func buildNativeRustArgumentFixtureFreeBSD(t *testing.T) string {
	t.Helper()
	...
	cmd := exec.Command("rustc",
		...
		"-C", "link-arg=-Wl,-z,now",
		"-o", output,
		filepath.Join("..", "testdata", "rust", "native_args.rs"),
	)
...
func buildNativeLifecycleFixtureFreeBSD(t *testing.T) string {
	t.Helper()
	target, ok := zigTargetFor("freebsd", runtime.GOARCH)
	if !ok {
		t.Fatalf("unsupported FreeBSD lifecycle target %s", runtime.GOARCH)
	}
...
func nativeLifecycleRecordWordFreeBSD(record []byte, index int) uintptr {
	wordSize := int(unsafe.Sizeof(uintptr(0)))
...
//go:nocheckptr
func copyNativeCallbackPayloadFreeBSD(data uintptr, size int32) ([]byte, bool) {
	if data == 0 || size < 0 {
		return nil, false
	}
