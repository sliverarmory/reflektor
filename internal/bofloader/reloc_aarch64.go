...
func applyARM64Branch19(location []byte, target, place uint64, explicit bool, explicitAddend int64) error {
	...
	if delta&3 != 0 {
		return fmt.Errorf("conditional branch target displacement %d is not 4-byte aligned", delta)
	}
	if delta < -(1<<20) || delta > (1<<20)-4 {
		return fmt.Errorf("conditional branch target displacement %d exceeds signed 21-bit range", delta)
	}
...
func arm64LoadStoreScale(word uint32) uint {
	...
	if word&(1<<26) != 0 && (word>>22)&0x3 == 0x3 {
		return 4
	}
